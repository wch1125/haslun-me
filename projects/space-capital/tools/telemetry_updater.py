...
def pct_true(series: pd.Series) -> float:
    ...
    if s.empty:
        return 0.0
    # TradingView exports sometimes use 1/0 or True/False or strings
    if s.dtype == bool:
        return float(s.mean())
    try:
...
class MarketSummary:
    ...
    # Extra context (optional but useful)
    volatility: float
    avg_return: float
    max_drawdown: float
...
def compute_kernel_respect(df: pd.DataFrame) -> float:
    ...
    # normalize by robust scale of returns (so tickers are comparable)
    rets = close.pct_change().to_numpy()
    scale = rolling_mad(rets[~np.isnan(rets)]) or np.nanstd(rets) or 1e-6
    # Convert diff to "within X sigmas" measure
    # smaller diff relative to scale = higher respect
    within = diff < (close.abs().to_numpy() * scale * 2.0)  # 2 "sigma-ish"
    return clamp01(float(np.nanmean(within)))


def compute_band_compression(df: pd.DataFrame) -> float:
    """
    Use multi-band columns if present. Many exports have G100/G150/G200.
    ...
    if g100 and g200:
        ...
        # fallback: use intrabar range as a proxy for "expansion"
...
def compute_macd_persistence(df: pd.DataFrame) -> float:
    ...
    if sign.size < 10:
        return 0.0
    # Persistence: fraction of time the sign matches the majority sign
...
def compute_stop_hunt(df: pd.DataFrame) -> float:
    ...
    # Stop "density" (cap for readability)
...
def compute_volume_reliability(df: pd.DataFrame) -> float:
    ...
    # Reliability: low dispersion => high score
    dispersion = np.nanstd(ratio)
    # Map dispersion to 0..1 with a soft knee
    score = 1.0 / (1.0 + dispersion)  # dispersion 0 => 1, dispersion 1 => 0.5
    return clamp01(float(score))


def compute_signal_follow_through(df: pd.DataFrame) -> float:
    """
    Approximate: of Buy/Sell signals, how often price moves favorably within N bars
    without being stopped. This is intentionally simple and robust.
    """
    buy = safe_col(df, "Buy")
    ...
    # Normalize signal markers: sometimes NaN/1, sometimes text
    if buy:
    ...
    target = 0.012  # 1.2% favorable move is "follow-through"
    outcomes: List[bool] = []

    # Buy follow-through: max future close / entry - 1 >= target
    for i in buy_idx:
        if i + 1 >= close.size: 
            continue
    ...
    # Sell follow-through: entry / min future close - 1 >= target
    for i in sell_idx:
        if i + 1 >= close.size:
            continue
        end = min(close.size, i + 1 + horizon)
...
def main():
    ...
    ap.add_argument("--csv-dir", required=True, help="Folder containing downloaded TradingView CSVs")
    ap.add_argument("--repo", required=True, help="Path to game repo 'trading' folder (or repo root)")
    ap.add_argument("--out-rel", default="data/market_summaries", help="Output path relative to repo root")
    ap.add_argument("--commit", action="store_true", help="Run git add/commit after writing summaries")
    ap.add_argument("--push", action="store_true", help="Also git push (implies --commit)")
    args = ap.parse_args()

    repo = os.path.abspath(args.repo)
    # allow passing either repo root or /trading folder
    if os.path.basename(repo) == "trading":
        repo_root = os.path.dirname(repo)
        trading_root = repo
    else:
        repo_root = repo
        trading_root = os.path.join(repo_root, "trading")

    out_dir = os.path.join(trading_root, args.out_rel)
